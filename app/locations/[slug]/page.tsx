...
export default async function LocationPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const isOrangeCounty = location.slug === "orange-county";
  const breadcrumb = {
  ...
  return <><PageHero eyebrow={location.eyebrow} title={`Insurance support for ${location.name}`}>{location.summary}</PageHero>
    ...
    <section className="section"><div className="shell service-layout"><div><p className="lead-small">{location.intro}</p><h2>Coverage needs commonly reviewed</h2><ul className="service-list">{location.localNeeds.map((item) => <li key={item}>{item}</li>)}</ul></div>{isOrangeCounty?<aside className="details-card"><h2>Local access</h2><p><strong>Devan Wright</strong><br/>Agent / Broker<br/>CA Lic. No. 0H19544</p><p>23621 La Palma Ave., Suite A<br/>Yorba Linda, CA 92887</p><p><strong>Main office:</strong> <a className="text-link" href="tel:7147016411" aria-label="Call Devan Wright’s Yorba Linda main office at 714-701-6411">714-701-6411</a><br/><strong>Direct/text:</strong> <a className="text-link" href="tel:7147016412" aria-label="Call Devan Wright directly at 714-701-6412">714-701-6412</a></p><div className="profile-links"><a className="text-link" href="sms:7147016412" aria-label="Text Devan Wright at 714-701-6412">Text Devan →</a><a className="text-link" href="mailto:devan.aduran@farmersagency.com" aria-label="Email Devan Wright">devan.aduran@farmersagency.com</a><a className="text-link" href="https://maps.google.com/?q=23621+La+Palma+Ave+Suite+A+Yorba+Linda+CA+92887" target="_blank" rel="noreferrer" aria-label="Get directions to Devan Wright’s Yorba Linda office">Get directions ↗</a><Link className="button" href="/contact?agent=devan&market=Orange%20County">Request Local Quote Support</Link></div></aside>:<aside className="details-card"><h2>Local access</h2><p>{location.officeNote}</p><p><strong>Direct:</strong> <a className="text-link" href={siteConfig.phoneHref}>{siteConfig.directPhone}</a></p><p><strong>CA License:</strong> {siteConfig.license}</p><div className="profile-links"><Link className="button" href={`/contact?market=${encodeURIComponent(location.name)}`}>Request Local Quote Support</Link><Link className="text-link" href={location.officeHref}>{location.officeLabel} →</Link></div></aside>}</div></section>
